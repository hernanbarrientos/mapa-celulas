...
import { Search, MapPin, Calendar, User, Users, Loader2, Map as MapIcon, List, Navigation, UserCheck, X, Moon, Sun } from 'lucide-react';
...
// --- ÍCONE PERSONALIZADO DO WHATSAPP (SVG OFICIAL) ---
...
export default function MapaBase({ modoLider = false }) {
  ...
  // --- RENDER POPUP (DESIGN PREMIUM WHATSAPP) ---
    ...
    <div className="min-w-[240px] text-left bg-white">
       ...
       <div className="p-4 text-gray-700 space-y-2.5 text-sm leading-relaxed">
          ...
          <div className="flex items-start gap-2">
             ...
             <div>
                <span className="block text-gray-700 leading-tight">
                   {formatEnderecoCompleto(celula)} - {celula.bairro}
                </span>
         ...
         <div className="flex flex-col gap-2 mt-4 pt-2 border-t border-gray-100">
           {modoLider ? (
              <>
                  {/* BOTÕES DE LÍDERES (VERDE SUAVE WHATSAPP) */}
                  <div className="grid grid-cols-2 gap-2">
                    {celula.lider1_whatsapp && (
                        <a href={`https://wa.me/${celula.lider1_whatsapp}`} target="_blank" className="flex items-center justify-center gap-1.5 py-2.5 rounded-lg text-xs font-bold bg-green-700 !text-white border border-green-800 hover:bg-green-900 transition-colors no-underline text-center shadow-sm">
                            <WhatsAppIcon className="w-4 h-4" /> {celula.lider1_nome?.split(' ')[0] || 'Líder 1'}
                        </a>
                    )}
                    {celula.lider2_whatsapp && (
                        <a href={`https://wa.me/${celula.lider2_whatsapp}`} target="_blank" className="flex items-center justify-center gap-1.5 py-2.5 rounded-lg text-xs font-bold bg-green-700 !text-white border border-green-800 hover:bg-green-900 transition-colors no-underline text-center shadow-sm">
                            <WhatsAppIcon className="w-4 h-4" /> {celula.lider2_nome?.split(' ')[0] || 'Líder 2'}
                        </a>
                    )}
                  </div>

                  {/* BOTÕES DE SUPERVISORES (AZUL SUAVE - Stacked) */}
                  {celula.supervisores && (
        ...
        <div className="space-y-2 relative z-10 pl-2 text-white/90 text-sm">
            ...
            <div className="flex items-start gap-2">
                ...
                <div>
                    ...
                    <p className="text-xs text-white/70 line-clamp-2">{formatEnderecoCompleto(celula)}</p>
                </div>
            </div>
            <div className="flex items-center gap-2">
      ...
      <div className={`w-full md:w-[450px] md:shrink-0 flex-col h-full bg-white dark:bg-gray-900 shadow-2xl z-20 transition-all ${mobileView === 'map' ? 'hidden md:flex' : 'flex'}`}>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-50 dark:bg-gray-900 pb-24 scrollbar-thin dark:scrollbar-thumb-gray-700">
          {loading ? <div className="flex justify-center p-10"><Loader2 className="animate-spin text-blue-600"/></div> : 
           celulas.length === 0 ? <div className="text-center text-gray-400 mt-10">Nenhuma célula encontrada. <br/>Tente buscar apenas o nome do bairro.</div> : 
           celulas.filter((celula) => {
      ...
      <div className={`flex-1 h-full relative bg-gray-200 dark:bg-gray-800 ${mobileView === 'list' ? 'hidden md:flex' : 'flex'}`}>
        <MapContainer ref={mapRef} center={[-23.691, -46.564]} zoom={13} className="h-full w-full z-10 outline-none">
          ...
          {celulas.filter((celula) => {
            ...
            .filter((celula) => selectedCategory ? celula.categoriaId === selectedCategory : true).map((celula) => (
